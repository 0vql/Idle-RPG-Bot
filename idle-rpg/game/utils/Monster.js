...
class Monster extends BaseHelper {
  ...
  generateMonster(selectedPlayer) {
    ...
    const mobAmount =
      mobAmountChance >= 75
        ? this.randomBetween(1, Math.floor((selectedPlayer.level * Math.log(1.2)) / 2) + 1)
        : 1;
    const mobList = [];
    for (let currentAmount = 0; currentAmount < mobAmount; currentAmount++) {
      ...
      const monsterObj = {
        ...
        health:
          monsterRarityList[randomRarityIndex].health + monsterTypeList[randomTypeIndex].health,
        maxHealth:
          monsterRarityList[randomRarityIndex].health + monsterTypeList[randomTypeIndex].health,
        stats: {
          str:
          ...
          int:
            selectedPlayer.level <= 5
              ? (monsterRarityList[randomRarityIndex].stats.int *
                  ...
                  selectedPlayer.stats.int / 1.2) /
                2
              : monsterRarityList[randomRarityIndex].stats.int *
                  monsterTypeList[randomTypeIndex].stats.int +
                selectedPlayer.stats.int / 1.2,
          luk:
            monsterRarityList[randomRarityIndex].stats.luk *
            monsterTypeList[randomTypeIndex].stats.luk,
        },
        dmgDealt: 0,
        dmgReceived: 0,
        power:
          monsterRarityList[randomRarityIndex].power * monsterTypeList[randomTypeIndex].power +
          playerBalance,
        ...
        experience: Math.ceil(
          (monsterRarityList[randomRarityIndex].experience *
            monsterTypeList[randomTypeIndex].experience) /
            2,
        ),
        gold: Math.round(
          monsterRarityList[randomRarityIndex].gold * monsterTypeList[randomTypeIndex].gold,
        ),
      };
