...
class Luck extends aggregation(BaseGame, BaseHelper) {
    ...
    try {
      ...
      switch (luckEvent) {
        case 1:
          ...
          eventMsg.push(
            `Hades unleashed his wrath upon ${this.generatePlayerName(updatedPlayer, true)} making ${this.generateGenderString(updatedPlayer, 'him')} lose ${luckExpAmount} experience!`,
          );
          eventLog.push(
        ...
        case 2:
          ...
          eventMsg.push(
            `${this.generatePlayerName(updatedPlayer, true)} was struck down by a thunderbolt from Zeus and lost ${luckHealthAmount} health because of that!`,
          );
          eventLog.push(
        ...
        case 3:
          ...
          if (healthDeficit) {
            ...
            eventMsg.push(
              `Fortune smiles upon ${this.generatePlayerName(updatedPlayer, true)} as Aseco cured ${this.generateGenderString(updatedPlayer, 'his')} sickness and restored ${this.generateGenderString(updatedPlayer, 'him')} ${healAmount} health!`,
            );
          ...
          eventMsg.push(
            `Aseco gave ${this.generatePlayerName(updatedPlayer, true)} an elixir of life but it caused no effect on ${this.generateGenderString(updatedPlayer, 'him')}. Actually it tasted like wine!`,
          );
        ...
        case 4:
          ...
          if (updatedPlayer.gold.current < goldTaken || goldTaken === 0) {
            eventMsg.push(
              `Hermes demanded some gold from ${this.generatePlayerName(updatedPlayer, true)} but as ${this.generateGenderString(updatedPlayer, 'he')} had no money, Hermes left him alone.`,
            );
            eventLog.push('Hermes demanded gold from you but you had nothing to give');
            break;
          }
          eventMsg.push(
            `Hermes took ${goldTaken} gold from ${this.generatePlayerName(updatedPlayer, true)} by force. Probably he is just out of humor.`,
          );
          eventLog.push(
        ...
        case 5:
          ...
          eventMsg.push(
            `Athena shared her wisdom with ${this.generatePlayerName(updatedPlayer, true)} making ${this.generateGenderString(updatedPlayer, 'him')} gain ${luckExpAthena} experience!`,
          );
          eventLog.push(
        ...
        case 6:
          ...
          if (updatedPlayer.spells.length > 0) {
            ...
            if (shouldAddToList) {
              ...
              eventMsg.push(
                `Eris has given ${this.generatePlayerName(updatedPlayer, true)} a scroll containing \`${spell.name}\` to add to ${this.generateGenderString(updatedPlayer, 'his')} spellbook!`,
              );
        ...
        case 7:
          ...
          eventMsg.push(
            `Dionysus has partied with ${this.generatePlayerName(updatedPlayer, true)} increasing ${this.generateGenderString(updatedPlayer, 'his')} multiplier by ${increaseMult} for ${Math.floor(timeLimit / 60000)} minutes!`,
          );
          eventLog.push(
  ...
  async questEvent(playerObj, mob, isCommand) {
    ...
    eventMsg.push(
      `[\`${updatedPlayer.map.name}\`] Quest Master has asked ${this.generatePlayerName(updatedPlayer, true)} to kill ${updatedPlayer.quest.questMob.count === 1 ? 'a' : updatedPlayer.quest.questMob.count} ${mob}!`,
    );
    eventLog.push(
  ...
  async goldEvent(playerObj, multiplier) {
    ...
    try {
      ...
      if (luckGoldChance >= 75) {
        ...
        eventMsg.push(
          `[\`${updatedPlayer.map.name}\`] ${this.generatePlayerName(updatedPlayer, true)} found ${goldAmount} gold!`,
        );
