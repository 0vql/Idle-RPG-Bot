...
class Battle extends aggregation(BaseGame, BaseHelper) {
    ...
    try {
      ...
      switch (battleResults.result) {
        case enumHelper.battle.outcomes.win:
          ...
          const checkedWinResults = await this.checkExperience(
        ...
        case enumHelper.battle.outcomes.lost:
          const checkLostResults = await this.checkHealth(
            this.Database,
            this.MapManager,
            updatedPlayer,
            battleResults.updatedMob,
  ...
  async camp(updatedPlayer) {
    ...
    const eventLog = [];
    try {
      updatedPlayer = await this.passiveRegen(
        updatedPlayer,
  ...
  async playerVsMobResults(results, multiplier) {
    try {
      ...
      if (isQuestCompleted) {
        eventMsg.push(
          `${this.generatePlayerName(updatedPlayer, true)} finished a quest and gained an extra ${questExpGain} exp and ${questGoldGain} gold!`,
        );
        eventLog.push(
  ...
  async findPlayerToBattle(playerObj, onlinePlayers) {
    ...
    const mappedPlayers = await this.Database.getSameMapPlayers(
    ...
    if (updatedPlayer.equipment.weapon.name !== enumHelper.equipment.empty.weapon.name) {
      const sameMapPlayers = mappedPlayers.filter(
        (player) =>
          ...
          onlinePlayers.findIndex((onlinePlayer) => onlinePlayer.discordId === player.discordId) !==
            -1 &&
      ...
      const playersWithBounty = sameMapPlayers
        ...
        .map(
          (player) => (player.chance = Math.floor((player.currentBounty * Math.log(1.2)) / 100)),
        );
      ...
      if (sameMapPlayers.length > 0 && updatedPlayer.health > (100 + updatedPlayer.level * 5) / 4) {
        ...
        if (playersWithBounty.length > 0 && this.randomBetween(0, 99) >= 50) {
          ...
          if (filteredBountyPlayers.length > 0) {
            const filteredBountyPlayersIndex = this.randomBetween(
              0,
  ...
  async playerVsPlayer(playerObj, playerToBattle, multiplier) {
    ...
    try {
      const { attacker, defender, attackerDamage, defenderDamage } =
        await this.BattleSimulator.simulateBattle(updatedPlayer, playerToBattle);
...
↳ ${this.capitalizeFirstLetter(this.generateGenderString(attacker, 'he'))} dealt \`${attackerDamage}\` dmg, received \`${defenderDamage}\` dmg! [HP:${attacker.health}/${playerMaxHealth}]-[${this.generatePlayerName(defender, true)} HP:${defender.health}/${defenderMaxHealth}]`,
      ...
      switch (result) {
        case enumHelper.battle.outcomes.win:
          ...
          const victimCheckHealth = await this.checkHealth(
            this.Database,
            this.MapManager,
          ...
          const winnerCheckExp = await this.checkExperience(
            victimCheckHealth.updatedAttacker,
            eventMsg,
            eventLog,
          );
        ...
        case enumHelper.battle.outcomes.fled:
          const fledUpdatedDefender = await this.checkExperience(
            defender,
            eventMsg,
            otherPlayerPmMsg,
          );
        ...
        case enumHelper.battle.outcomes.lost:
          ...
          const lostUpdatedDefender = await this.checkExperience(
            loseResults.stealingPlayer,
            eventMsg,
            otherPlayerPmMsg,
          );
          await this.Database.savePlayer(lostUpdatedDefender.updatedPlayer);
          const loserCheckHealth = await this.checkHealth(
            this.Database,
            this.MapManager,
      ...
      if (luckStealChance > 90 - canSteal) {
        ...
        if (
          ...
          if (victimPlayer.equipment[itemKeys[luckItem]].previousOwners.length > 0) {
            ...
            const removePreviousOwnerName = victimPlayer.equipment[itemKeys[luckItem]].name.replace(
              `${lastOwnerInList}`,
              `${victimPlayer.name}`,
            );
            ...
            otherPlayerLog.push(
              `${stealingPlayer.name}${stealingPlayer.titles.current !== 'None' ? ` the ${stealingPlayer.titles.current}` : ''} stole ${victimPlayer.equipment[itemKeys[luckItem]].name} from you`,
            );
          } else {
            stolenEquip = victimPlayer.equipment[itemKeys[luckItem]];
          ...
          if (
            ...
            const oldItemRating = await this.calculateItemRating(
              stealingPlayer,
              stealingPlayer.equipment[itemKeys[luckItem]],
            );
            const newItemRating = await this.calculateItemRating(
              victimPlayer,
              victimPlayer.equipment[itemKeys[luckItem]],
            );
            if (oldItemRating < newItemRating) {
              stealingPlayer = await this.setPlayerEquipment(
                stealingPlayer,
              ...
              stealingPlayer = await this.InventoryManager.addEquipmentIntoInventory(
                stealingPlayer,
                stolenEquip,
              );
            }
            if (
              ...
              const equipFromInventory = victimPlayer.inventory.equipment
                .filter(
                  (equipment) =>
                    equipment.position === enumHelper.equipment.types[itemKeys[luckItem]].position,
                )
                .sort((item1, item2) => item2.power - item1.power)[0];
              victimPlayer = await this.setPlayerEquipment(
                victimPlayer,
                enumHelper.equipment.types[itemKeys[luckItem]].position,
                equipFromInventory,
              );
  ...
  async dropItem(playerObj, mob, eventMsg, eventLog) {
    ...
    try {
      ...
      if (dropitemChance <= 15 + updatedPlayer.stats.luk / 4) {
        const item = await this.ItemManager.generateItem(
          updatedPlayer,
        ...
        if (item.position !== enumHelper.inventory.position) {
          ...
          if (oldItemRating > newItemRating) {
            updatedPlayer = await this.InventoryManager.addEquipmentIntoInventory(
              updatedPlayer,
              item,
            );
          } else {
            updatedPlayer = await this.setPlayerEquipment(
              updatedPlayer,
        ...
        if (!item.holiday) {
          eventMsg.push(
            `${this.generatePlayerName(updatedPlayer, true)} received \`${item.name}\` from \`${mob.find((obj) => obj.health <= 0).name}!\``,
          );
        } else {
          eventMsg.push(
            `**${this.generatePlayerName(updatedPlayer, true)} received \`${item.name}\` from \`${mob.find((obj) => obj.health <= 0).name}!\`**`,
