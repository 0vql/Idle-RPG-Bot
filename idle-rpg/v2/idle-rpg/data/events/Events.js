...
class Events extends BaseHelper {
  ...
  async moveEvent(playerObj) {
    ...
    try {
      ...
      eventMsg.push(
        `${this.generatePlayerName(updatedPlayer)} decided to head \`${mapObj.direction}\` from \`${updatedPlayer.previousMap}\` and arrived in \`${mapObj.map.name}\`.`,
      );
      eventLog.push(
  ...
  async luckEvent(loadedPlayer, events, globalMultiplier) {
    ...
    try {
      ...
      return this.LuckEvents.goldEvent(
        updatedPlayer,
        globalMultiplier + updatedPlayer.personalMultiplier,
      );
    } catch (err) {
