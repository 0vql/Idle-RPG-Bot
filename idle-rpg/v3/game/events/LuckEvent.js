...
class LuckEvent {
    ...
    try {
      ...
      switch (luckEvent) {
        case 1: {
          ...
          eventMsg.push(
            `Hades unleashed his wrath upon ${generatePlayerName(updatedPlayer, true)} making ${generateGenderString(updatedPlayer, 'him')} lose ${luckExpAmount} experience!`,
          );
          eventLog.push(
            `Hades unleashed his wrath upon you making you lose ${luckExpAmount} experience`,
          );
          break;
        }
        case 2: {
          ...
          eventMsg.push(
            `${generatePlayerName(updatedPlayer, true)} was struck down by a thunderbolt from Zeus and lost ${luckHealthAmount} health!`,
          );
          eventLog.push(
            `Zeus struck you down with his thunderbolt and you lost ${luckHealthAmount} health`,
          );
          break;
        }
        case 3: {
          ...
          if (healthDeficit) {
            ...
            eventMsg.push(
              `Fortune smiles upon ${generatePlayerName(updatedPlayer, true)} as Aseco cured ${generateGenderString(updatedPlayer, 'his')} sickness and restored ${generateGenderString(updatedPlayer, 'him')} ${healAmount} health!`,
            );
          ...
          eventMsg.push(
            `Aseco gave ${generatePlayerName(updatedPlayer, true)} an elixir of life but it caused no effect on ${generateGenderString(updatedPlayer, 'him')}. Actually it tasted like wine!`,
          );
        ...
        case 4: {
          ...
          if (updatedPlayer.gold.current < goldTaken || goldTaken === 0) {
            eventMsg.push(
              `Hermes demanded some gold from ${generatePlayerName(updatedPlayer, true)} but as ${generateGenderString(updatedPlayer, 'he')} had no money, Hermes left him alone.`,
            );
            eventLog.push('Hermes demanded gold from you but you had nothing to give');
            break;
          }
          eventMsg.push(
            `Hermes took ${goldTaken} gold from ${generatePlayerName(updatedPlayer, true)} by force. Probably he is just out of humor.`,
          );
          eventLog.push(
            `Hermes took ${goldTaken} gold from you. It will be spent in favor of Greek pantheon. He promises!`,
          );
        ...
        case 5: {
          ...
          eventMsg.push(
            `Athena shared her wisdom with ${generatePlayerName(updatedPlayer, true)} making ${generateGenderString(updatedPlayer, 'him')} gain ${luckExpAthena} experience!`,
          );
          eventLog.push(
            `Athena shared her wisdom with you making you gain ${luckExpAthena} experience`,
          );
        ...
        case 6: {
          ...
          if (!shouldAddToList) {
            const dupIdx = updatedPlayer.spells.findIndex(
              (ownedSpell) =>
                spell.name.includes(ownedSpell.name.split(/ (.+)/)[1]) &&
                spell.power > ownedSpell.power,
            );
            if (dupIdx !== -1) {
          ...
          if (shouldAddToList) {
            ...
            eventMsg.push(
              `Eris has given ${generatePlayerName(updatedPlayer, true)} a scroll containing \`${spell.name}\` to add to ${generateGenderString(updatedPlayer, 'his')} spellbook!`,
            );
        ...
        case 7: {
          ...
          eventMsg.push(
            `Dionysus has partied with ${generatePlayerName(updatedPlayer, true)} increasing ${generateGenderString(updatedPlayer, 'his')} multiplier by ${increaseMult} for ${Math.floor(timeLimit / 60000)} minutes!`,
          );
          eventLog.push(
            `Dionysus partied with you increasing your multiplier by ${increaseMult} for ${Math.ceil(timeLimit / 60000)} minutes!`,
          );
          updatedPlayer.personalMultiplier = increaseMult;
          setTimeout(() => {
            this.db
              .loadPlayer(updatedPlayer.discordId)
              .then((loadedPlayer) => {
        ...
        case 8: {
          ...
          if (updatedPlayer.spells.length > 0 && updatedPlayer.mana < maxMana) {
            ...
            eventMsg.push(
              `Apollo's light fills ${generatePlayerName(updatedPlayer, true)} with arcane energy, restoring ${manaRestored} mana!`,
            );
            ...
            eventMsg.push(
              `Apollo whispered ancient knowledge to ${generatePlayerName(updatedPlayer, true)}, granting ${generateGenderString(updatedPlayer, 'him')} ${apolloExp} experience!`,
            );
        ...
        case 9: {
          ...
          eventMsg.push(
            `Poseidon's tides swept ${generatePlayerName(updatedPlayer, true)} away from \`${prevMap}\` to \`${town.name}\`!`,
          );
        ...
        case 10: {
          ...
          eventMsg.push(
            `Artemis rewarded ${generatePlayerName(updatedPlayer, true)} for ${generateGenderString(updatedPlayer, 'his')} bravery with a gift from the hunt!`,
          );
  ...
  async questEvent(playerObj, mob, isCommand) {
    ...
    eventMsg.push(
      `[\`${updatedPlayer.map.name}\`] Quest Master has asked ${generatePlayerName(updatedPlayer, true)} to kill ${updatedPlayer.quest.questMob.count === 1 ? 'a' : updatedPlayer.quest.questMob.count} ${mob}!`,
    );
    eventLog.push(
      `Quest Master in ${updatedPlayer.map.name} asked you to kill ${updatedPlayer.quest.questMob.count === 1 ? 'a' : updatedPlayer.quest.questMob.count} ${mob}.`,
    );
    await this.player.logEvent(updatedPlayer, eventLog[0], enumHelper.logTypes.action);
    return { type: 'actions', updatedPlayer, msg: eventMsg, pm: eventLog };
  }
  ...
  async goldEvent(playerObj, multiplier) {
    ...
    try {
      ...
      const goldAmount = Math.max(
        1,
        Math.round((luckGoldDice * updatedPlayer.stats.luk) / 2) * multiplier,
      );
      updatedPlayer.gold.current += goldAmount;
      updatedPlayer.gold.total += goldAmount;
      eventMsg.push(
        `[\`${updatedPlayer.map.name}\`] ${generatePlayerName(updatedPlayer, true)} found ${goldAmount} gold!`,
      );
  ...
  async catchSnowFlake(playerObj, snowFlake) {
    ...
    try {
      ...
      if (snowFlakeDice <= 5) {
        ...
        if (oldItemRating < newItemRating) {
          eventMsg.push(
            `<@!${updatedPlayer.discordId}> **just caught a strange looking snowflake within the blizzard!**`,
          );
          eventLog.push(
