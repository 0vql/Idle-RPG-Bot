...
class AttackEvent {
  ...
  async sell(playerObj) {
    ...
    try {
      if (updatedPlayer.inventory.equipment.length > 0) {
        ...
        eventMsg.push(
          `[\`${updatedPlayer.map.name}\`] ${generatePlayerName(updatedPlayer, true)} just sold what they found adventuring for ${profit} gold!`,
        );
  ...
  async generateItemEvent(playerObj) {
    ...
    try {
      ...
      eventMsg.push(
        `[\`${updatedPlayer.map.name}\`] ${generatePlayerName(updatedPlayer, true)} just purchased \`${item.name}\` for ${itemCost} gold!`,
      );
