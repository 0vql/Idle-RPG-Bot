...
class BattleEngine {
    ...
    try {
      ...
      switch (battleResults.result) {
        case enumHelper.battle.outcomes.win: {
          const dropItemResults = await this.dropItem(
            updatedPlayer,
          ...
          const checkedWinResults = await this.player.checkExperience(
        ...
        case enumHelper.battle.outcomes.fled: {
          const checkedFledResults = await this.player.checkExperience(
            updatedPlayer,
            eventMsg,
            eventLog,
          );
        ...
        case enumHelper.battle.outcomes.lost: {
          const checkLostResults = this.player.checkHealth(
            updatedPlayer,
            battleResults.updatedMob,
            eventMsg,
            eventLog,
  ...
  async camp(updatedPlayer) {
    ...
    const eventLog = [];
    try {
      updatedPlayer = this.player.passiveRegen(
        updatedPlayer,
      ...
      await this.player.logEvent(
        updatedPlayer,
        generatedMessage.eventLog,
  ...
  async playerVsMobResults(results, multiplier, guildEvents) {
    try {
      ...
      const formatResult = this._pveMessageFormat(
        results,
        results.attacker,
        playerMaxHealth,
        effectiveMultiplier,
      ...
      if (isQuestCompleted) {
        eventMsg.push(
          `${generatePlayerName(updatedPlayer, true)} finished a quest and gained an extra ${questExpGain} exp and ${questGoldGain} gold!`,
        );
        eventLog.push(
  ...
  _pveMessageFormat(results, updatedPlayer, playerMaxHealth, multiplier) {
    ...
    results.defender.forEach((mob) => {
      ...
      if (infoList !== -1) {
        ...
        mobListInfo.mobs.push({
          mob: mob.name,
          totalCount: 0,
          event: { killed: 0, fled: 0, survived: 0 },
        });
        ...
        if (mobTypeName) {
          const playerNameStr =
            updatedPlayer.titles.current !== 'None'
              ? `${updatedPlayer.name} the ${updatedPlayer.titles.current}`
              : updatedPlayer.name;
    ...
    if (mobFleeCountString) {
      eventMsg.push(
        results.attackerDamage > results.defenderDamage
          ? `${mobFleeCountString} just fled from ${generatePlayerName(results.attacker, true)}!`.replace(
              /1x /g,
              '',
            )
          : `${generatePlayerName(results.attacker, true)} just fled from ${mobFleeCountString}!`.replace(
              /1x /g,
              '',
            ),
      );
    ...
    if (mobKillCountString) {
      eventMsg.push(
        `${generatePlayerName(updatedPlayer, true)}'s \`${updatedPlayer.equipment.weapon.name}\` just killed ${mobKillCountString}`.replace(
          /1x /g,
          '',
        ),
      );
    ...
    const attackedMsg =
      `Attacked ${mobCountString.replace(/`/g, '')} with \`${updatedPlayer.equipment.weapon.name}\` in \`${updatedPlayer.map.name}\` `.replace(
        /1x /g,
        '',
      );
  ...
  async findPlayerToBattle(playerObj, onlinePlayers) {
    ...
    const mappedPlayers = await this.db.getSameMapPlayers(
      updatedPlayer.guildId,
      updatedPlayer.map.name,
    );
      ...
      if (sameMapPlayers.length > 0 && updatedPlayer.health > (100 + updatedPlayer.level * 5) / 4) {
        ...
        if (playersWithBounty.length > 0 && randomBetween(0, 99) >= 50) {
          ...
          if (filteredBountyPlayers.length > 0) {
            randomPlayer =
              filteredBountyPlayers[randomBetween(0, filteredBountyPlayers.length - 1)]._doc;
          } else {
  ...
  async playerVsPlayer(playerObj, playerToBattle, multiplier) {
    ...
    try {
      const { attacker, defender, attackerDamage, defenderDamage } =
        await this.simulator.simulateBattle(updatedPlayer, playerToBattle);
      ...
      if (attacker.health <= 0 && defender.health > 0) {
        battleResultLog = battleResultLog.replace(
          `  ${attacker.name} has ${attacker.health}/${playerMaxHealth} HP left.`,
          '',
        );
        eventMsg.push(
          `[\`${attacker.map.name}\`] ${generatePlayerName(defender, true)} just killed ${generatePlayerName(attacker, true)} with ${generateGenderString(defender, 'his')} \`${defender.equipment.weapon.name}\`!\n↳ ${generatePlayerName(attacker, true)} dealt \`${attackerDamage}\` dmg, received \`${defenderDamage}\` dmg! [${generatePlayerName(defender, true)} HP:${defender.health}/${defenderMaxHealth}]`,
        );
        ...
        eventLog.push(
          `Attacked ${defender.name} in ${attacker.map.name} with ${attacker.equipment.weapon.name} and dealt ${attackerDamage} damage! [${expGainAttacker} exp]`,
        );
      ...
      switch (result) {
        case enumHelper.battle.outcomes.win: {
          ...
          const victimCheckHealth = this.player.checkHealth(
          ...
          const winnerCheckExp = await this.player.checkExperience(
            victimCheckHealth.updatedAttacker || winResults.stealingPlayer,
            eventMsg,
            eventLog,
          );
          return {
        ...
        case enumHelper.battle.outcomes.fled: {
          const fledUpdatedDefender = await this.player.checkExperience(
            defender,
            eventMsg,
            otherPlayerPmMsg,
          );
        ...
        case enumHelper.battle.outcomes.lost: {
          ...
          const lostUpdatedDefender = await this.player.checkExperience(
            loseResults.stealingPlayer,
            eventMsg,
            otherPlayerPmMsg,
          );
          await this.db.savePlayer(lostUpdatedDefender.updatedPlayer);
          const loserCheckHealth = this.player.checkHealth(
  ...
  async steal(stealingPlayer, victimPlayer) {
    ...
    try {
      ...
      if (luckStealChance > stealThreshold) {
        ...
        if (
          ![enumHelper.equipment.empty.armor.name, enumHelper.equipment.empty.weapon.name].includes(
            victimEquip.name,
          )
        ) {
          let stolenEquip;
          if (victimEquip.previousOwners.length > 0) {
            const lastOwnerInList =
              victimEquip.previousOwners[victimEquip.previousOwners.length - 1];
            stolenEquip = victimEquip;
            stolenEquip.name = victimEquip.name.replace(
              `${lastOwnerInList}`,
              `${victimPlayer.name}`,
            );
          } else {
          ...
          eventMsg.push(
            setImportantMessage(
              `${stealingPlayer.name}${stealingPlayer.titles.current !== 'None' ? ` the ${stealingPlayer.titles.current}` : ''} just stole ${stolenEquip.name}!`,
            ),
          );
          eventLog.push(`Stole ${victimEquip.name}`);
          otherPlayerLog.push(
            `${stealingPlayer.name}${stealingPlayer.titles.current !== 'None' ? ` the ${stealingPlayer.titles.current}` : ''} stole ${victimEquip.name} from you`,
          );
          victimPlayer.stolen++;
          stealingPlayer.stole++;
          if (victimEquip.name !== enumHelper.equipment.empty[itemSlot].name) {
            const oldItemRating = calculateItemRating(
              stealingPlayer,
              stealingPlayer.equipment[itemSlot],
            );
            const newItemRating = calculateItemRating(stealingPlayer, victimEquip);
            if (oldItemRating < newItemRating) {
              stealingPlayer = this.player.setPlayerEquipment(
                stealingPlayer,
                enumHelper.equipment.types[itemSlot].position,
                stolenEquip,
              );
              if (victimEquip.previousOwners.length > 0) {
              ...
              stealingPlayer = this.inventory.addEquipmentIntoInventory(
                stealingPlayer,
                stolenEquip,
              );
            }
            if (
              ...
              victimPlayer.inventory.equipment.find(
                (equip) => equip.position === enumHelper.equipment.types[itemSlot].position,
              ) !== undefined
            ) {
              const equipFromInventory = victimPlayer.inventory.equipment
                .filter(
                  (equipment) =>
                    equipment.position === enumHelper.equipment.types[itemSlot].position,
                )
                .sort((item1, item2) => item2.power - item1.power)[0];
              victimPlayer = this.player.setPlayerEquipment(
                victimPlayer,
                enumHelper.equipment.types[itemSlot].position,
                equipFromInventory,
              );
              ...
              victimPlayer = this.player.setPlayerEquipment(
                victimPlayer,
                enumHelper.equipment.types[itemSlot].position,
                enumHelper.equipment.empty[itemSlot],
              );
            }
          }
        }
        return {
          ...
          msg: eventMsg,
          pm: eventLog,
          otherPlayerPmMsg: otherPlayerLog,
        };
      }
      if (victimPlayer.gold.current > victimPlayer.gold.current / 6) {
        ...
        if (goldStolen !== 0) {
          ...
          eventMsg.push(
            setImportantMessage(
              `${stealingPlayer.name}${stealingPlayer.titles.current !== 'None' ? ` the ${stealingPlayer.titles.current}` : ''} just stole ${goldStolen} gold from ${victimPlayer.name}${victimPlayer.titles.current !== 'None' ? ` the ${victimPlayer.titles.current}` : ''}!`,
            ),
          );
          eventLog.push(`Stole ${goldStolen} gold from ${victimPlayer.name}`);
          otherPlayerLog.push(
            `${stealingPlayer.name}${stealingPlayer.titles.current !== 'None' ? ` the ${stealingPlayer.titles.current}` : ''} stole ${goldStolen} gold from you`,
          );
          return {
            stealingPlayer,
            victimPlayer,
  ...
  async dropItem(playerObj, mob, eventMsg, eventLog, guildEvents) {
    ...
    try {
      ...
      if (dropitemChance <= 15 + updatedPlayer.stats.luk / 4) {
        ...
        const isInvasionActive =
          guildEvents && guildEvents.isInvasionActive && guildEvents.invasionMobType;
        const item =
          isInvasionActive && randomBetween(0, 99) < 2
        ...
        if (item.position !== enumHelper.inventory.position) {
          ...
          if (oldItemRating > newItemRating) {
            ...
            updatedPlayer = this.player.setPlayerEquipment(
              updatedPlayer,
        ...
        if (!item.holiday) {
          eventMsg.push(
            `${generatePlayerName(updatedPlayer, true)} received \`${item.name}\` from \`${deadMob.name}!\``,
          );
        } else {
          eventMsg.push(
            `**${generatePlayerName(updatedPlayer, true)} received \`${item.name}\` from \`${deadMob.name}!\`**`,
        ...
        if (LEGENDARY_RARITY_NAMES.has(item.name.split(' ')[0])) {
          const playerNameStr =
            updatedPlayer.titles.current !== 'None'
              ...
              : updatedPlayer.name;
          eventMsg.unshift(
            setImportantMessage(`${playerNameStr} just found a legendary ${item.name}!`),
          );
        }
