    ...
    try {
      if (updatedPlayer.experience.current >= updatedPlayer.level * 15) {
        ...
        if (MILESTONES.has(updatedPlayer.level)) {
          eventMsg.push(
            setImportantMessage(`${updatedPlayer.name} has reached level ${updatedPlayer.level}!`),
          );
        }
        for (let i = 0; i < 4; i++) {
        ...
        const playerStats = Object.keys(updatedPlayer.stats)
          .map((key) => {
            if (['str', 'dex', 'int'].includes(key))
              return { key, value: updatedPlayer.stats[key] };
          })
