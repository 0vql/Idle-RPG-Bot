...
module.exports = [
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      if (isNaN(index))
        return author.send('Please specify an item index. Usage: `!equips <number>`');
      ...
      if (!player.inventory.equipment.length)
        return author.send('Your equipment inventory is empty!');
      const item = player.inventory.equipment[index - 1];
      if (!item)
        return author.send(
          `No item found at index ${index}. You have ${player.inventory.equipment.length} item(s).`,
        );
      ...
      const oldItem = { ...player.equipment[item.position] };
      ...
      if (
        oldItem.name !== enumHelper.equipment.empty[item.position] &&
        oldItem.name !== enumHelper.equipment.empty.weapon.name &&
        oldItem.name !== enumHelper.equipment.empty.armor.name
      ) {
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      if (currentItem.name === enumHelper.equipment.empty[slot].name)
        return author.send(`Your ${slot} slot is already empty.`);
      if (player.inventory.equipment.length >= enumHelper.inventory.maxEquipmentAmount)
        return author.send('Your equipment inventory is full!');
      player.inventory.equipment.push({ ...currentItem });
      player.equipment[slot] = { ...enumHelper.equipment.empty[slot] };
      await game.db.savePlayer(player);
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      const player = await game.db.loadPlayer(author.id, { pastEvents: 0, pastPvpEvents: 0 });
      if (!player) return author.send('You have not been born yet!');
      if (!player.inventory.equipment.length && !player.inventory.items.length)
        return author.send('Your inventory is empty!');
      let profit = 0;
      player.inventory.equipment.forEach((equip) => {
        profit += Number(equip.gold) || 0;
      });
      player.inventory.items.forEach((item) => {
        profit += Number(item.gold) || 0;
      });
  ...
  },
  {
    ...
    handler: async ({ game, bot, message, guildId, author }) => {
      ...
      if (player.gold.current < itemCost)
        return author.send(
          `You need ${itemCost} gold to buy this item, but you only have ${player.gold.current} gold.`,
        );
      if (item.name.startsWith('Cracked'))
        return author.send('No good items available right now. Try again later!');
      if (item.position !== enumHelper.inventory.position) {
        ...
        } else if (player.inventory.equipment.length < enumHelper.inventory.maxEquipmentAmount) {
          player.gold.current -= itemCost;
        ...
        if (player.inventory.items.length >= enumHelper.inventory.maxItemAmount)
          return author.send('Your item inventory is full!');
        player.gold.current -= itemCost;
