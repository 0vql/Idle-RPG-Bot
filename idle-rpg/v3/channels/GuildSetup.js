...
class GuildSetup {
  ...
  async manageGuildChannels(guild) {
    ...
    if (!botGuildMember.permissions.has(PermissionFlagsBits.ManageChannels)) {
      console.log(
        `Idle RPG does not have permission to manage channels in guild: ${guild.name} (${guild.id})`,
      );
      return;
    }
    ...
    for (const channel of guild.channels.cache.values()) {
      if (channel.type === ChannelType.GuildCategory && channel.name.toLowerCase() === 'idle-rpg') {
        ...
        channel.parent &&
        channel.parent.name.toLowerCase() === 'idle-rpg'
      ) {
        switch (channel.name.toLowerCase()) {
          case 'leaderboards':
            hasLeaderboardsChannel = channel;
            break;
          case 'commands':
            hasCommandsChannel = channel;
            break;
          case 'faq':
            hasFAQChannel = channel;
            break;
          case 'actions':
            hasActionsChannel = channel;
            break;
          case 'movement':
            hasMovementChannel = channel;
            break;
        }
      }
    }
    ...
    if (!categoryChannel) {
      ...
      try {
        ...
        categoryChannel = guild.channels.cache.find(
          (channel) => channel.name.toLowerCase() === 'idle-rpg',
        );
      } catch (err) {
        console.log(err);
      }
    ...
    if (!hasCommandsChannel && categoryChannel) {
      ...
      try {
        ...
        await commandsChannel.setTopic(
          'In order to easier check other players stats/equip, a command channel was created.',
          'Setting up Idle-RPG Channels',
        );
      } catch (err) {
        console.log(err);
      }
