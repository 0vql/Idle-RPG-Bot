    ...
    try {
      const commandChannelId =
        (await messageObj.channel.type) === 'dm'
          ...
          : messageObj.guild.channels.cache.find(
              (channel) => channel.name.toLowerCase() === 'commands' && channel.type === 'text',
            ).id;
      const guildId =
      ...
      if (commandRegex.test(command)) {
        const commandObj = await commands.filter((c) =>
          c.command instanceof Array ? c.command.includes(command) : c.command === command,
        )[0];
        if (!commandObj) {
          return;
        }
